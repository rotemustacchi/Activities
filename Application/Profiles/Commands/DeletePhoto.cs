    ...
    public class DeletePhoto
    {
            ...
            : IRequestHandler<Command, Result<Unit>>
        {
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if(photo.Id==user.ImageUrl) return Result<Unit>.Failure("You cannot delete your main photo",400);
                await photoService.DeletePhoto(photo.PublicId);
                user.Photos.Remove(photo);

                var result = await context.SaveChangesAsync(cancellationToken)>0;
                return result ? Result<Unit>.Success(Unit.Value) : Result<Unit>.Failure("Cannot delete main Photo ",400);
            }
        }
    }
}
